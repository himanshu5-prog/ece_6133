...
void generateCSV(map<int, pair<double,double>> finalPlacement)
{
...
void generateRowData(Row *row, int rowCount)
{
	for(int i=0; i<rowCount; i++)
	{
		cout<<"Row id #"<<i+1<<" ";
		cout<<"rowPointer: "<<row[i].rowPointer<<" ";
		cout<<" Row y_coord "<<row[i].y_coord<<" ";
		cout<<"cell count: "<<row[i].cell_list.size()<<endl;
