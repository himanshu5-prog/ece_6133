...
void FM_Algorithm_single_pass_3(vector<Cell> cell_list, int cell_count, vector<int> cell_net_list[], vector<Cell> detailed_net_list[], int net_count ,
vector<Cell> adjacent_list[], int max_connect, vector<Cell> global_cell_list, map<int,int> gain, int **netDistr,
vector<int> &movement, vector<int> &movementGain, vector<int> &movementGainSum);

void FM_Algorithm_wrapper (vector<Cell> &cell_list, int cell_count, vector<int> cell_net_list[], vector<Cell> detailed_net_list[], int net_count , int initial_cutsize,
int threshold,vector<Cell> adjacent_list[], int max_connect, vector<Cell> &global_cell_list, vector<int> net_list[], double width, double height,
double x_coord, double y_coord, int mode,bool p_type);

void FM_Algorithm_single_pass_4(vector<Cell> cell_list, int cell_count, vector<int> cell_net_list[], vector<Cell> detailed_net_list[], int net_count, 
vector<Cell> adjacent_list[], int max_connect, vector<Cell> global_cell_list, map<int,int> &gain, int **netDistr,
vector<int> &movement, vector<int> &movementGain, vector<int> &movementGainSum)
{
	...
	int threshold1 = (0.1 * cell_count) + 2;
	...
	//		cout<<"Found 1907\n";
		...
		assert(cell_id_g >=1);
		bucket_ds_2[gainCell].insert(cell_id_g);
	}
	...
	while(get_unlocked_cell_id_5(cell_list)!=-1)
	{
			...
			//	cout<<bucket_ds_2[h].size()<<endl;
				...
				//	or(int y=0; y < bucket_ds[h].size();y++)
					...
					{
					//	cout<<"h: "<<h<<endl;
						//cell_id = bucket_ds[h][y];
						cell_id = *it;
						if(cell_id == 0 || cell_id == -1)
							continue;
						assert(cell_id >= 1);
						
						current_p_id = cell_list[cell_id - 1].partition_id;
			...
			//			cout<<"Active cell: "<<base_cell<<" gain: "<<max_gain<<endl;
						...
						cell_list[cell_id - 1].locked=true;
					...
					//	cout<<"locked active cell\n";
						...
						assert(bucket_ds_2[h].find(cell_id) != bucket_ds_2[h].end());
						
						bucket_ds_2[h].erase(cell_id);
						//if(cell_id == 1907)
						//	cout<<"deleted 1907\n";
						...
						{
							net_id = cell_net_list[cell_id - 1][x];
							...
							else if (cell_list[base_cell - 1].partition_id ==2)
							{
								ts = 0;
								fs = 1;
							}
							...
							if(netDistr[net_id][ts] == 0)
							{
								for(int p = 0; p<detailed_net_list[net_id].size(); p++)
								{
									...
									if(cell_list[cell_id1 - 1].locked == false && global_cell_list[cell_id1-1].active == true)
									{
										old_gain = gain[cell_id1];
										assert(bucket_ds_2[old_gain].find(cell_id1) != bucket_ds_2[old_gain].end());
										bucket_ds_2[old_gain].erase(cell_id1);
										//vector<int>::iterator it;
					...
					//					cout<<"ts: Incrementing gain of "<<cell_id1<<" initial gain: "<<old_gain<<" to "<<gain[cell_id1]<<endl;
										...
										bucket_ds_2[old_gain + 1].insert(cell_id1);
									}
								}	
							}
							else if (netDistr[net_id][ts] == 1)
							{
								for(int p = 0; p<detailed_net_list[net_id].size(); p++)
								{
									...
									if(cell_list[cell_id1 - 1].locked == false && cell_list[cell_id1 - 1].partition_id == dest_p_id && 
									global_cell_list[cell_id1-1].active == true)
									{
										old_gain = gain[cell_id1];
										//vector<int>::iterator it;
										//it = find(bucket_ds[old_gain+max_connect].begin(), bucket_ds[old_gain+max_connect].end(),cell_id1 );
					...
					//					cout<<"ts: decrementing gain of "<<cell_id1<<" initial gain: "<<old_gain<<" to "<<gain[cell_id1]<<endl;
							...
							cell_list[base_cell - 1].partition_id = dest_p_id;
							//------------------------------------------------------
					...
					//					cout<<"ts: decrementing gain of "<<cell_id1<<" initial gain: "<<old_gain<<" to "<<gain[cell_id1]<<endl;
							...
							else if( netDistr[net_id][fs] == 1)
							{
								for(int p = 0; p<detailed_net_list[net_id].size(); p++)
								{
									...
									if(cell_list[cell_id1 - 1].locked == false && cell_list[cell_id1 - 1].partition_id == current_p_id &&
									global_cell_list[cell_id1-1].active == true)
									{
										old_gain = gain[cell_id1];
										//vector<int>::iterator it;
										...
										//assert(it != bucket_ds[old_gain + max_connect].end());
										//bucket_ds[old_gain + max_connect].erase(it);
										if(bucket_ds_2[old_gain].find(cell_id1) == bucket_ds_2[old_gain].end())
										{
											cout<<"gain: "<<gain[cell_id1]<<" size: "<<bucket_ds_2[old_gain].size()<<endl;
...
void FM_Algorithm_single_pass(vector<Cell> cell_list, int cell_count, vector<int> cell_net_list[], vector<Cell> detailed_net_list[], int net_count , int initial_cutsize
...
vector<Cell> global_cell_list, double x_coord, double y_coord, double width, double height, int mode, double delta, map<int,int> gain, int **netDistr,
vector<int> &movement, vector<int> &movementGain, vector<int> &movementGainSum)

{
	...
	while(get_unlocked_cell_id_5(cell_list)!=-1)
	{
			...
			{
				exit = false;
				if(!bucket_ds[h].empty())
				{
				//	cout<<"h: "<<h<<endl;
					for(int y=0; y < bucket_ds[h].size();y++)
					{
						//cout<<"h: "<<h<<endl;
						cell_id = bucket_ds[h][y];
						assert(cell_id >= 1);
						...
						for(int x = 0; x < cell_net_list[base_cell - 1].size(); x++)
						{
							...
							if(netDistr[net_id][ts] == 0)
							{
								for(int p = 0; p<detailed_net_list[net_id].size(); p++)
								{
									...
									if(cell_list[cell_id1 - 1].locked == false)
									{
										old_gain = gain[cell_id1];
...
void FM_Algorithm_single_pass_2(vector<Cell> cell_list, int cell_count, vector<int> cell_net_list[], vector<Cell> detailed_net_list[], int net_count , int initial_cutsize
,vector<Cell> adjacent_list[], int initial_cut_size, int max_connect, vector<Cell> &cell_ptr_p1, vector<Cell> &cell_ptr_p2,bool p_type,
vector<Cell> global_cell_list, double x_coord, double y_coord, double width, double height, int mode, double delta, map<int,int> gain, int **netDistr,
vector<int> &movement, vector<int> &movementGain, vector<int> &movementGainSum)

{
	...
	{
		bucket_ds[gain[i+1] + max_connect].push_back(i+1);
		bucket_ds_2[gain[i+1]].insert(i+1);
	}
	//-------------------
...
//	cout<<"initial gain computed\n";
	...
	do
		...
		FM_Algorithm_single_pass_4(cell_list,cell_count,cell_net_list,detailed_net_list ,net_count,adjacent_list,max_connect, 
		global_cell_list, gain, netDistribution,movement,movementGain,movementGainSum);
		...
		for(int i=0; i<= max_pos;i++)
		{
			...
			if(cell_list[cell_id2 - 1].partition_id == 1 )
			{
				...
				//global_cell_list[cell_id2 - 1].partition_id =2;
			}
		...
		new_cut_size = calculate_cutsize_5(detailed_net_list,net_count);
	...
	//	cout<<"Negative diff!!\n";
		...
		int new_cut_size = calculate_cutsize_5(detailed_net_list,net_count);
	//	cout<<"new cut size: "<<new_cut_size<<endl;
